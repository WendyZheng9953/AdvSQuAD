...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            adv_init_mag = 1e-1  # todo: set a pertubation bound
            norm_type = "l2"  # todo: set a norm type (N_theta)
            adv_steps = 2  # todo: set K
            adv_max_norm = 0
            adv_lr = 2e-5

            # todo: initialize delta
            if adv_init_mag > 0:
                input_mask = inputs['attention_mask'].to(embeds_init)  # change type
                input_lengths = torch.sum(input_mask, 1)

                if norm_type == "l2":
                    delta = torch.zeros_like(embeds_init).uniform_(-1,1) * input_mask.unsqueeze(
                        2)  # set tensor to random then add another dim
                    dims = input_lengths * embeds_init.size(-1)
                    mag = adv_init_mag / torch.sqrt(dims)
                    delta = (delta * mag.view(-1, 1, 1)).detach()
                elif norm_type == "linf":
                    delta = torch.zeros_like(embeds_init).uniform_(-adv_init_mag, adv_init_mag) * input_mask.unsqueeze(2)
            ...
            for astep in range(adv_steps):
                ...
                # todo: Get delta gradient
                delta_grad = delta.grad.clone().detach()

                # todo: Update pertubation
                if norm_type == "l2":
                    ...
                    if args.adv_max_norm > 0:
                        delta = torch.clamp(delta, -adv_max_norm, args.adv_max_norm).detach()
...
def main():
    ...
    parser.add_argument(
        "--output_dir",
        default="../models/wwm_uncased_finetuned_squad/",
        type=str,
        required=False,
    ...
    '''
    tokenizer =transformers.BertTokenizer.from_pretrained("./bert-base-uncased/vocab.txt")
